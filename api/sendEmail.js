export default async function handler(req, res) {
  ...
  const { email, tierTitle, tierBadge, chain, txHash } = req.body;
  if (!email || !tierTitle) return res.status(400).json({ error: 'Missing fields' });

  const txLine = txHash
    ? `<p style="margin:8px 0;color:#888">Transaction: <span style="color:#00e5ff">${txHash.slice(0,24)}...${txHash.slice(-8)}</span></p>`
    : '';
